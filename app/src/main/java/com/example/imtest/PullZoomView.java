 ...
 * ================================================
 */
public class PullZoomView extends VerticalScrollView {
    ...
    private ViewGroup.MarginLayoutParams headerParams;//头部的参数
    ...
    private int MaxMoveY = maxY;                   //允许最大滑动距离,默认划出屏幕
    private int setmaxy;                   //设置滑动距离值
    ...
    public void setMaxMoveY(final int maxy) {
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        headerParams = (ViewGroup.MarginLayoutParams) headerView.getLayoutParams();
...
//                        } else {
                        isZooming = true;
//                        }
                        if (contentView.getTop() < MaxMoveY) {
                            if (height > MaxMoveY) {
                                headerParams.height = MaxMoveY;
                                headerView.setLayoutParams(headerParams);
                            } else if(headerView.getHeight()<MaxMoveY-setmaxy*2){
//                                headerParams.height = MaxMoveY-400;
                            } else {
...
//                        if (Math.abs(height) < MaxMoveY - setmaxy && Math.abs(height) > setmaxy) {
//                            headerParams.height = Math.abs(height);
//                        }
                    }
                }
                break;
